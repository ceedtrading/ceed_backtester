        ...
        for i, bar in future.iterrows():
            if side == 'Buy':
                ...
                if bar['Low'] <= current_stop:
                    exit_p, status = "BE" if be_activated else "Loss"
                    break
        ...
        if exit_p is not None or status != "Loss":
            ...
            if df_lead is not None:
                ...
                if not lead_snap.empty:
                    ...
                    lead_drift = "Aligned" if (side == 'Buy' and drift > 0) or (side == 'Sell' and drift < 0) else "Friction"
    ...
    if st.button("Run Gemini 3 Synthetic Review"):
        ...
        if not results.empty:
            ...
            with st.spinner("Engaging Gemini 3 Flash Engine..."):
                # Clean ASCII summary for the new SDK
